...
- **`.github/workflows/pkgdown.yaml`** : construit et déploie
  ...
  sur la branche par défaut (voir `_pkgdown.yml` pour la configuration
  de l’index de référence) ; il sera consultable à
...
- **`.gitignore`** : complété pour exclure `.DS_Store`, ainsi que deux
  ...
  — ils ne seront donc pas poussés sur GitHub une fois que vous aurez
  lancé `git add`. Si vous souhaitez au contraire verser une vraie photo
  d’exemple ou des données digitalisées réelles au dépôt, retirez les
  lignes correspondantes du `.gitignore` (idéalement sous un chemin
  dédié comme `data-raw/` plutôt qu’à la racine).
