...
digitization: a trained ml-morph model proposes the 19 FISHMORPH
landmarks from a few calibration clicks per specimen, which are then
reviewed, corrected and exported to `CSV`/`tpsDig`. It requires a live
interactive session (and an external ml-morph resource directory holding
the trained predictor and a `dlib`-enabled Python environment), so it
cannot be run inside this vignette, but the intended usage is:

``` r

# Open the app (auto-detecting an "ml_morph/" resource directory):
digitize_landmarks()

# Import the exported table into an "intrait_landmarks" object:
