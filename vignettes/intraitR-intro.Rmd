...
morphological ratios, shape space, intraspecific variability, and
...
If landmarks have not yet been digitized, [digitize_landmarks()] launches
a bundled Shiny application for predictor-assisted digitization. The
session is declared at the console -- the photograph folder, the output
workbook, the operator, the working queue -- and everything it produces
goes to one workbook (a `measurements` sheet, a `bias` sheet holding the
repeated digitizations, and a `bias_summary` sheet quantifying them),
backed by an append-only journal that survives a crash. A trained ml-morph
model can propose the 19 FISHMORPH landmarks from a few clicks per
specimen, which are then reviewed and corrected. It requires a live
interactive session (and, for the prediction step only, an external
ml-morph resource directory with the trained predictor and a
`dlib`-enabled Python environment), so it cannot be run inside this
vignette, but the intended usage is:
...
`FALSE` to skip that step. As with `shape_space()`, `plot()` defaults to
